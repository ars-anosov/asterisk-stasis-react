...
module.exports.Stasis = function(ariAsterClient, stasisName, mysqlConnection) {
	...
	this.stasisStart = function (event, channel) {
		...
		// WireShark filter : http.request.uri matches "/ari/channels/1492076173.8030"
		...
		if (event.args[0] === 'leg2') {
			var oldTime = event.args[1]+'.'+event.args[2];
			var newTime = ''+process.hrtime();
			oldTime = parseFloat(oldTime);
			newTime = parseFloat(newTime.replace(",", "."));

			console.log('leg1-leg2 delay: '+(newTime-oldTime));
			...
			playbackFile(channel, 'sound:pls-hold-while-try', onPlaybackFinished, startTime);
			//getDataFromMysql(channel, onMysqlDataReady, startTime);
		}
		...
		function playbackFile(channel, sound, callback /* calback(channel) */, startTime) {
			...
			playback.on('PlaybackFinished', function (event, playback) {
				if (self.wsServerConn.headers) { self.wsServerConn.sendText( JSON.stringify(event || {}, null, 2) ); }
				if (calback_pass && callback) { callback(channel, startTime); }
		...
		function onPlaybackFinished(channel, startTime) {
			getDataFromMysql(channel, onMysqlDataReady, startTime);
		}
		...
		function getDataFromMysql(channel, callback /* calback(channel, result) */, startTime) {
			...
			callback(channel, result, startTime);
		}
		
		function onMysqlDataReady(channel, mysqlResult, startTime) {
